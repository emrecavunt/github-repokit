locals {
  ...
  default_github_provider_attribute_condition = join(" && ", concat(
    trimspace(var.github_owner) != "" ? ["assertion.repository_owner==\"${var.github_owner}\""] : [],
    length(local.trusted_github_repositories) > 0 ? [
      "(${join(" || ", [for repo in local.trusted_github_repositories : "assertion.repository==\"${repo}\""])})"
    ] : []
  ))
